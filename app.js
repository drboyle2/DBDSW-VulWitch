...
function runFlawfinderAndGeneratePDF(code, filename, callback) {
    ...
    const sanitizeFilePath = (filePath) => {
      return filePath.replace(/[;&|`$\\]/g, ''); // remove potentially dangerous characters
    };
    
    const sanitizedFilePath = sanitizeFilePath(codeFilePath);

    // Run Flawfinder on the uploaded code
    exec(`flawfinder ${sanitizedFilePath}`, (error, stdout, stderr) => {
        if (error) {
